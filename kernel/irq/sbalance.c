...
static void sbalance_wait(long poll_jiffies)
{
	...
	/*
	 * Wait using a deferrable timer so we don't kick CPUs out of idle.
	 * Use the freezer_do_not_count()/freezer_count() pattern (the 5.10
	 * equivalent of TASK_FREEZABLE) to avoid blocking system suspend.
	 */
